...
When the user wants to explore alternative interfaces for a chosen deepening candidate, use this comparison workflow. Based on "Design It Twice" (Ousterhout): your first idea is unlikely to be the best.
...
Show this to the user, then proceed to Step 2 without requiring confirmation when the constraints are already settled.
...
Develop each design from the same technical brief (file paths, coupling details, dependency category from [DEEPENING.md](DEEPENING.md), what sits behind the seam). The brief is independent of the user-facing problem-space explanation in Step 1. Give each design a different constraint:
