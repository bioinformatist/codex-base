...
| improve | Improve 顾问、协调器与审查器 | 是 | 是 | 插件中显式调用 `$codex-base:improve ...`，或在 Nix 环境调用 `$improve ...`；正式规划使用具备原生上下文管理和结构化提问能力的内置 Plan Mode，而实现、审计和普通生命周期记录不受此规划门槛限制；如暂时缺少原生上下文管理，请参阅须由用户授权的[单计划临时豁免](../README.zh-CN.md#temporary-context-waiver) | 先查清事实，只询问仍未确定且影响实质结果的选择；在对话中给出完整的替换计划且不写文件；仅在之后获准的阶段持久化或委派；逐步简化并管理审查与检查点边界 | 以只读方式运行 `$codex-base:improve quick`；Nix 提供 `codex-improve` 和 `$improve` | 改编自 [shadcn-improve](https://github.com/shadcn/improve)，由 Codex Base 维护 |
| executor-routing | 按已审查的计划选择执行通道 | 是 | 是 | 已审查的 `.17` 计划列出准确路径、检查和执行通道的依据 | 协调器按获准通道选择 economy（Luna/low）、standard（Sol/medium）或 deep（Sol/xhigh）；scout 使用 Luna/high，审查使用 Sol/high。每次调用固定一个角色，不查询额度、不自动换模，也不重放已启动的调用 | 检查 `src/improve/config/roles.json`、规划契约的 Executor routing 章节及 `tests/improve/test_runtime.py` | Codex Base |
